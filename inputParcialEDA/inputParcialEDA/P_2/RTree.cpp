...
bool RTree::InsertRectRec(const Branch& a_branch, Node* a_node, Node** a_newNode, int a_level)
{
     ...
     Node* N = a_node;
     // cambiar codicion
     while (N->level != a_level){
            bool hijoshoja = (N->level == a_level + 1);
            if (hijoshoja) {
                InsertRect(a_branch,a_node, a_level)
                N = true;
            }
            else
                N = false;
  ...
  // Escribe tu implementacion aqui, debes corregir la linea 43 que indica return false
  return false;
}
